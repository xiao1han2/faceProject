...
datetime: 2024/1/7 21:10
desc:训练脚本，由于数据集不大，这里一次性读入内存
"""
import argparse
...
if opt.dataset == "fer2013":
    ...
    callback = [
        ...
        ModelCheckpoint('../models/cnn2_best_weights_fer.h5', monitor='val_acc', verbose=True, save_best_only=True,
                        save_weights_only=True)]
    model.save_weights('../models/cnn3_best_weights_fer.h5')
    ...
    print("picture_test accuacy", np.sum(pred.reshape(-1) == y_test.reshape(-1)) / y_test.shape[0])
    ...
    callback = [
        ...
        ModelCheckpoint('../models/cnn3_best_weights_jaffe.h5', monitor='val_accuracy', verbose=True, save_best_only=True,
                        save_weights_only=True)]
    model.save_weights('../models/cnn3_best_weights_jaffe.h5')
    ...
    callback = [
        #     EarlyStopping(monitor='val_loss', patience=50, verbose=True),
        #     ReduceLROnPlateau(monitor='lr', factor=0.1, patience=15, verbose=True),
        # ModelCheckpoint('../models/cnn3_best_weights_ck.h5', monitor='val_acc', verbose=True, save_best_only=True,
        #                 save_weights_only=True)]
        ModelCheckpoint('../models/cnn3_best_weights_ck.h5', monitor='val_acc', verbose=True, save_best_only=True,
                        save_weights_only=True)]
    # model.save_weights('../models/cnn3_best_weights_ck.h5') #不受回调函数的印象，更灵活
